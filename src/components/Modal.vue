  ...
  const formatingIngredientList = () => {
    ...
    for(let i = 1; i <= 15; i++ ) {
      if(drinks.recipe[`strIngredient${i}`]) {
        ...
        const noNullQuantity =  quantity !== null ? quantity : 'Taste Preference'
        const ingredientQuantity = document.createElement('P')
        ingredientQuantity.classList.add('text-lg', 'text-gray-500')
        ingredientQuantity.textContent = `${ingredient} - ${noNullQuantity}`
              ...
              <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6" >
                <div>
                  <div class="mt-3">
                    ...
                    <div v-html="formatingIngredientList().outerHTML"></div>
